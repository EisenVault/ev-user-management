   ...
   Alfresco.ConsoleUsers = function(htmlId)
   {
      ...
      YAHOO.extend(SearchPanelHandler, Alfresco.ConsolePanelHandler,
      {
         ...
         onLoad: function onLoad()
         {
            ...
            /**
             * EisenVault - Monika Kumari - Start - 
             */
            Alfresco.util.Ajax.jsonGet(
            {
               url: Alfresco.constants.PROXY_URI + "/com/eisenvault/usersCount",
               successCallback:
               {
            	   	fn: function loadWebscript_successCallback(response, config)
               		{
            	   		console.log(response.json.returnStatus);
            	   		var n = "false".localeCompare(response.json.returnStatus);
            ...
            /**
             * EisenVault - Monika Kumari - End 
             */
            
            
            // make an ajax call to get authentication mutability -
			// "creationAllowed" will be returned as true
            // in the response if the administrator is able to create new users
         ...
         {
            // update the text field - as this event could come from bookmark,
			// navigation or a search button click
            var searchTermElem = Dom.get(parent.id + "-search-text");
            searchTermElem.value = parent.searchTerm;

            // check search length again as we may have got here via history
			// navigation
            if (!this.isSearching && parent.searchTerm !== undefined && parent.searchTerm.length >= parent.options.minSearchTermLength)
            {
               ...
               var failureHandler = function ConsoleUsers__ps_failureHandler(sRequest, oResponse)
               {
                  ...
                  {
                     // Our session has likely timed-out, so refresh to offer
						// the login page
               ...
               // ... with hint to use CQ for user admin page (note: passed via
				// searchTerm in lieu of a change in the /api/people API)
               ...
               // Disable search button and display a wait feedback message if
				// the users hasn't been found yet
         ...
         /**
			 * Enable search button, hide the pending wait message and set the
			 * panel as not searching.
			 * 
         ...
         {
            // Enable search button and close the wait feedback message if
			// present
         ...
         _setupDataTable: function _setupDataTable()
         {
			 ...
			 * 
			 * Each cell has a custom renderer defined as a custom function. See
			 * YUI documentation for details. These MUST be inline in order to
			 * have access to the parent instance (via the "parent" variable).
			 */
            ...
            var renderCellFullName = function renderCellFullName(elCell, oRecord, oColumn, oData)
            {
               if (!oRecord.getData("isDeleted"))
               {
                  ...
                  // fire the 'viewUserClick' event when the selected user in
            ...
            parent.widgets.pagingDataTable = new Alfresco.util.DataTable(
            {
               ...
               dataSource:
               {
                  ...
                  {
                     // People webscript uses other pagination parameters
         ...
         /**
			 * Resets the YUI DataTable errors to our custom messages NOTE:
			 * Scope could be YAHOO.widget.DataTable, so can't use "this"
			 * 
      ...
      YAHOO.extend(ViewPanelHandler, Alfresco.ConsolePanelHandler,
      {
         ...
         onBeforeShow: function onBeforeShow()
         {
			...
			// show
            // old data to the user before the Update() method paints the
			// results
         ...
         onUpdate: function onUpdate()
         {
            var success = function(res)
            {
               ...
               // build the company address up and set manually - encoding each
				// value
               var addr = "";
      ...
      YAHOO.extend(CreatePanelHandler, Alfresco.ConsolePanelHandler,
      {
         ...
         onLoad: function onLoad()
         {
            ...
            /*
			 * form.addValidation(parent.id + "-create-password",
			 * Alfresco.forms.validation.mandatory, null, "keyup");
			 * form.addValidation(parent.id + "-create-password",
			 * Alfresco.forms.validation.length, { min:
			 * parent.options.minPasswordLength, max: 100, crop: true,
			 * ignoreEmpty: true }, "change",
			 * parent._msg("Alfresco.forms.validation.length.message.min",
			 * parent.options.minPasswordLength)); form.addValidation(parent.id +
			 * "-create-verifypassword", Alfresco.forms.validation.mandatory,
			 * null, "keyup"); form.addValidation(parent.id +
			 * "-create-verifypassword", Alfresco.forms.validation.length, {
			 * min: parent.options.minPasswordLength, max: 100, crop: true,
			 * ignoreEmpty: true }, "change",
			 * parent._msg("Alfresco.forms.validation.length.message.min",
			 * parent.options.minPasswordLength));
			 */
         ...
         {
            // Inject the component from the XHR request into it's placeholder
			// DIV element
         ...
         /**
			 * Group selected event handler. This event can be fired from either
			 * Groups picker - so we much ensure the event is for the current
			 * panel by checking panel visibility.
			 * 
         ...
         onBeforeShow: function onBeforeShow()
         {
            // Hide the main panel area before it is displayed - so we don't
			// show
            // old data to the user before the onShow() method paints the
			// results
      ...
      YAHOO.extend(UpdatePanelHandler, Alfresco.ConsolePanelHandler,
      {
         ...
         addGroup: function addGroup(group)
         {
            ...
            if (!found)
            {
               ...
               // if this group wasn't one of the original list, then add it to
				// the addition list
               found = false;
               ...
               // if the group has been removed before, remove it from the
				// removed groups list
         ...
         onRemoveGroupUpdate: function onRemoveGroupUpdate(e, args)
         {
            ...
            if (group != null)
            {
               ...
               // if this group was one of the original list, then add it to
				// the removed list
               if (Alfresco.util.findInArray(this._originalGroups, group.itemName, "itemName"))
               {
         ...
         onUpdate: function onUpdate()
         {
            ...
            var success = function(res)
            {
               ...
               // add groups the user is already assigned to and maintain a
				// copy of the original group list
               me.resetGroups();
               YAHOO.Bubbling.fire("allItemsDeselected",
               {
               ...
               for (var i=0, j=person.groups.length; i<j; i++)
               {
                  ...
                  // Make the group finder aware of which groups the user
					// already has
               ...
               // Hide or show the old password field - only required if user
         ...
         onShow: function onShow()
         {
            if (parent.csvResults)
            {
               ...
               if (successful &&  successful.length > 0 && parent.csvResults.successful[0].response)
               {
                  ...
                  // If the response contains the "successful" array
					// containing an element then it does not necessarily
                  // mean that the CSV upload succeeded. This simply means
					// that the upload request was successfully processed
                  // (i.e. the file was received)
                  if (successful.data && successful.data.users)
                  {
                     parent.fileUpload.hide();

                     // If the successful response contains a data object
						// with a "users" attribute then we at least know that
                     // some users have been processed so can construct a
						// result table using that data...
   ...
   YAHOO.extend(Alfresco.ConsoleUsers, Alfresco.ConsoleTool,
   {
      ...
      options:
      {
         ...
         /*
			 ...
			 * 
			 * @property docsEdition @type String @default "community"
			 */
      ...
      onReauthorizedButtonClick: function ConsoleUsers_onReauthorizedButtonClick(e, cell, parent)
      {
         ...
         var success = function(res)
         {
            ...
            // Show uploader for single file select - override the upload URL to
			// use appropriate upload service
            var uploadConfig =
            {
            ...
            // Make sure the "use Flash" tip is hidden just in case Flash is
			// enabled...
      ...
      {
         // Force the use of the HTML (rather than Flash) uploader because
			// there are issues with the
         // Flash uploader in these circumstances when Sharepoint is being
			// used. The Flash uploader
         // picks up the wrong JSESSIONID cookie which causes the upload to
			// fail.
         if (!this.fileUpload)
         {
         ...
         {
            // Message of upload dialog was changed in reauthorize method. Get
			// back original messages
      ...
      /**
		 ...
		 * @param complete
		 *            {object} Object literal containing details of successful
		 *            upload
		 */
      ...
      {
         // return to the search screen - we can no longer view the user
			// details
      ...
      /**
		 ...
		 * @method _createUser
		 * @param handler
		 *            {function} Handler function to be called on successful
		 *            creation
		 * @private
		 */
      _createUser: function ConsoleUsers__createUser(handler)
      {
         ...
         {
            userName: username,
            password: "zegb2943tw9lf4t8setr84",
         ...
         Alfresco.util.Ajax.request(
         {
            ...
            failureCallback:
            {
               fn: function(res)
               {
                  ...
                  else if (res.serverResponse.status === 412)
                  {
                     // users limit exceeds
                     Alfresco.util.PopupManager.displayPrompt(
                     {
                        title: this._msg("message.failure"),
                        text: this._msg("Can't create. Users count exceeded")
                     });
                  }
                  else
                  {
      ...
      _updateUser: function ConsoleUsers__updateUser(handler)
      {
         ...
         if (YAHOO.lang.trim(password).length !== 0)
         {
            ...
            if(!password.match(values))
            {
            	Alfresco.util.PopupManager.displayMessage(
            	{
                   text: this._msg("Please enter a valid Password. Password must have 8 or more characters and at least one uppercase, one lowercase, one integer and one special character")
                });
             	return;
            }
         }
         ...
         // gather the groups for addition and groups for removal from the
			// panel
      ...
      /**
		 ...
		 * @param idPrefix
		 *            {string} ID prefix of the quota UI elements
		 * @return the quota value as input by the user - converted to bytes
		 * @private
		 */
